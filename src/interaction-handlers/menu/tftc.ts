    ...
    return new Promise((resolve) => {
        actionRowBuilder.addComponents(
            new StringSelectMenuBuilder()
                .setCustomId(`menu:tftc_a_${data?.join(",")}`)
                .setPlaceholder(
                ...
                .setOptions(
                    {
                        label: "1400 TFT Coins",
                        emoji: "1153924511977197610", // 1
                        value: "1400TFT:HIM",
                    },
                    {
                        label: "2325 TFT Coins",
                        emoji: "1153924511977197610", // 1
                        value: "2325TFT:HIM",
                    }
                )
                    ...
                    await interaction.update({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    "Has seleccionado `1400TFT Coins`. Para poder gestionar tu pedido se requiere el acceso a tu cuenta, así que primeramente escribe el método de inicio de sesión."
                                )
                            ...
                            interaction.channel.send({
                                embeds: [
                                    new EmbedBuilder()
                                        ...
                                        .setColor(Colors.Success)
                                        .setDescription(
                                            `Ahora escribe tu email o nombre de usuario.`
                                        ),
                                ],
                            });
                            ...
                            nameCollector.on("end", (collected, reason) => {
                                if (reason === "time") {
                                    ...
                                    const SkinEmbed = new EmbedBuilder()
                                        ...
                                        .setDescription(
                                            `Perfecto, tu email/usuario es \`${name}\` y tu método de inicio de sesión es \`${metodo}\`. Ahora, por favor, escribe la contraseña.`
                                        )
                                        .setColor(Colors.Success);
                                    ...
                                    imageCollector.on("collect", (message) => {
                                        ...
                                        interaction.channel.send({ content: `${Emojis.Misc.Loading}` }).then(async (msg) => {
                                            try {
                                                ...
                                                await module2.build(botone, { disabled: false, author: interaction.user.id }, [`${interaction.user.id}`, `${interaction.user.username}`, `${selectedOption}`, `${shortURL}`, `${UniqueID}`, `tftc`]);
                    ...
                    await interaction.update({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    "Has seleccionado `2325 TFT Coins`. Para poder gestionar tu pedido se requiere el acceso a tu cuenta, así que primeramente escribe el método de inicio de sesión."
                                )
                                .setColor(Colors.Success),
                        ],
                        components: [],
